/* $Id: errmsgwin-sorter.cpp 83743 2020-04-17 11:25:08Z vboxsync $ */
/** @file
...
int main(int argc, char **argv)
{
    ...
    if (pOut)
    {
        ...
        for (size_t i = 0; i < RT_ELEMENTS(g_aStatusMsgs); i++)
        {
            ...
            /* Make sure the message string doesn't contain problematic characters: */
            const char *pszMsgFull = pMsg->pszMsgFull;
            if (strpbrk(pszMsgFull, "\"\\"))
            {
                char *pszDst = s_szMsgTmp;
                char  ch;
                do
                {
                    ch = *pszMsgFull++;
                ...
                pszMsgFull = s_szMsgTmp;
            }

            /* Produce the output: */
            fprintf(pOut, "/*%#010lx:*/ { \"%s\", \"%s\", %ld },\n",
                    (unsigned long)pMsg->iCode, pszMsgFull, pMsg->pszDefine, pMsg->iCode);
        }
