/* $Id: isovfs.cpp 69029 2017-10-10 20:33:05Z vboxsync $ */
/** @file
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (pShared->Core.cbObject < RTFSISO_MAX_DIR_SIZE)
            {
                ...
                if (pShared->pbDir)
                {
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        Log3(("ISO/UDF: Directory content\n%.*RhxD\n", pShared->cbDir, pShared->pbDir));
#ifdef LOG_ENABLED
                        //rtFsIsoDirShrd_Log9660Content(pShared);
#endif
