/* $Id: VBoxClipboard.cpp 80359 2019-08-21 08:37:54Z vboxsync $ */
/** @file
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            for (;;)
            {
                ...
                rc = VbglR3ClipboardEventGetNext(&cmdCtx, pTransfer, &pEvent);
                if (RT_SUCCESS(rc))
                {
           ...
           if (RT_SUCCESS(rc))
           {
               if (uFormat == VBOX_SHARED_CLIPBOARD_FMT_BITMAP)
               {
                   ...
                   if (hClip != NULL)
                   {
                       ...
                       if (lp != NULL)
                       {
                           rc = VbglR3ClipboardWriteData(pCtx->CmdCtx.uClientID, VBOX_SHARED_CLIPBOARD_FMT_BITMAP,
                                                         lp, GlobalSize(hClip));
                           GlobalUnlock(hClip);
                       }
                       else
               ...
               else if (uFormat == VBOX_SHARED_CLIPBOARD_FMT_UNICODETEXT)
               {
                   ...
                   if (hClip != NULL)
                   {
                       ...
                       if (uniString != NULL)
                       {
                           rc = VbglR3ClipboardWriteData(pCtx->CmdCtx.uClientID, VBOX_SHARED_CLIPBOARD_FMT_UNICODETEXT,
               ...
               else if (uFormat == VBOX_SHARED_CLIPBOARD_FMT_HTML)
               {
                   ...
                   if (format != 0)
                   {
                       ...
                       if (hClip != NULL)
                       {
                           ...
                           if (lp != NULL)
                           {
                               rc = VbglR3ClipboardWriteData(pCtx->CmdCtx.uClientID, VBOX_SHARED_CLIPBOARD_FMT_HTML,
                                                             lp, GlobalSize(hClip));
                               GlobalUnlock(hClip);
                           }
                           else
...
DECLCALLBACK(int) VBoxClipboardInit(const PVBOXSERVICEENV pEnv, void **ppInstance)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        rc = VbglR3ClipboardConnect(&pCtx->CmdCtx.uClientID);
        if (RT_SUCCESS(rc))
        {
            pCtx->CmdCtx.uProtocol = 0;
...
DECLCALLBACK(int) VBoxClipboardWorker(void *pInstance, bool volatile *pfShutdown)
{
    ...
    for (;;)
    {
        ...
        uint32_t u32Msg;
        uint32_t u32Formats;
        rc = VbglR3ClipboardGetHostMsgOld(pCtx->CmdCtx.uClientID, &u32Msg, &u32Formats);
        if (RT_FAILURE(rc))
        {
