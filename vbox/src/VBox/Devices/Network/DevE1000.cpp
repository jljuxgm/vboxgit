/* $Id: DevE1000.cpp 44540 2013-02-05 13:02:18Z vboxsync $ */
/** @file
...
static DECLCALLBACK(int) e1kR3Construct(PPDMDEVINS pDevIns, int iInstance, PCFGMNODE pCfg)
{
    ...
    if (RT_FAILURE(rc))
        return rc;
    rc = PDMDevHlpCritSectInit(pDevIns, &pThis->csRx, RT_SRC_POS, "%sRX", pThis->szInstance);
    if (RT_FAILURE(rc))
        return rc;
#ifdef E1K_WITH_TX_CS
    rc = PDMDevHlpCritSectInit(pDevIns, &pThis->csTx, RT_SRC_POS, "%sTX", pThis->szInstance);
    if (RT_FAILURE(rc))
        return rc;
#endif /* E1K_WITH_TX_CS */
