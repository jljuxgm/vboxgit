/* $Id: DrvHostCoreAudio.cpp 60306 2016-04-04 15:22:23Z vboxsync $ */
/** @file
...
static int coreAudioReinitIn(PPDMIHOSTAUDIO pInterface, PPDMAUDIOHSTSTRMIN pHstStrmIn)
{
    ...
    if (RT_SUCCESS(rc))
    {
        rc = coreAudioInitIn(&pStreamIn->streamIn, NULL /* pcSamples */);
        if (RT_SUCCESS(rc))
...
static int coreAudioReinitOut(PPDMIHOSTAUDIO pInterface, PPDMAUDIOHSTSTRMOUT pHstStrmOut)
{
    ...
    if (RT_SUCCESS(rc))
    {
        rc = coreAudioInitOut(&pStreamOut->streamOut, NULL /* pcSamples */);
        if (RT_SUCCESS(rc))
