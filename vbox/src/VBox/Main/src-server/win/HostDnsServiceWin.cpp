/* $Id: HostDnsServiceWin.cpp 52899 2014-09-30 15:24:26Z vboxsync $ */
/** @file
...
const TCHAR HostDnsServiceWin::m_pwcKeyRoot[] = _T("SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters\\Interfaces");
...
bool HostDnsServiceWin::subscribeTo(TCHAR *wcsPath, TCHAR *wcsInterface, DWORD fFilter)
{
    HKEY   hTmpKey    = NULL;
    ...
    if (wcsPath)
    {
        ...
        rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, wcsPath, 0, KEY_READ | KEY_NOTIFY, &hTmpKey);
        if (rc == ERROR_SUCCESS)
        {
...
void HostDnsServiceWin::extendVectorWithStrings(std::vector<std::wstring>& pVectorToExtend, std::wstring &wcsParameter)
{
    std::wstringstream   wcsStream(wcsParameter);
    std::wstring         wcsSubString;

    while (std::getline(wcsStream, wcsSubString, _T(' ')))
        pVectorToExtend.push_back(wcsSubString);
}


#ifdef DEBUG
static void hostDnsWinDumpList(std::vector<std::wstring>& awcszValues)
{
...
HRESULT HostDnsServiceWin::updateInfo(uint8_t *fWhatsChanged)
{
    ...
    if (m_aWarehouse.size() > VBOX_OFFSET_SUBTREE_EVENTS)
    {
        ...
        for (int idxKey = VBOX_OFFSET_SUBTREE_EVENTS; idxKey < m_aWarehouse.size(); idxKey++)
        {
            ...
            DWORD cValues = 0;
            rc = RegQueryInfoKey(m_aWarehouse[idxKey].hKey, NULL, NULL, NULL, NULL, NULL, NULL, &cValues, NULL, NULL, NULL, NULL);
            if (rc == ERROR_SUCCESS)
            {
                for (DWORD idxValue = 0; idxValue < cValues; idxValue++)
                {
                    TCHAR wcsValueName[VBOX_KEY_NAME_LEN_MAX];
                    DWORD cbValueName = VBOX_KEY_NAME_LEN_MAX;
                    TCHAR wcsData[VBOX_KEY_NAME_LEN_MAX];
                    ...
                    if (rc == ERROR_SUCCESS)
                    {
                            ...
                            && _tcslen(wcsData) > 0)
                        {
                            ...
                            if (pHostDnsInfo.domain.empty())
                                pHostDnsInfo.domain = std::wstring(wcsData);
                        }
                        else if ((   _tcscmp(wcsValueName, _T("NameServer")) == 0
                                  || _tcscmp(wcsValueName, _T("DhcpNameServer")) == 0)
                                 && _tcslen(wcsData) > 0)
                        {
                            extendVectorWithStrings(pHostDnsInfo.servers, std::wstring(wcsData));
                        }
                        else if (_tcscmp(wcsValueName, _T("SearchList")) == 0
                                 && _tcslen(wcsData) > 0)
                        {
        ...
        {
#ifdef DEBUG
            LogRel(("Domain changed: [%ls]->[%ls].\n",
                m_hostInfoCache.domain.empty() ? _T("NONE") : (const unsigned short *)m_hostInfoCache.domain.data(),
                pHostDnsInfo.domain.data()));
#endif /* DEBUG */
