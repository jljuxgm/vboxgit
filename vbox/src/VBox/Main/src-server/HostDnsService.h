/* $Id: HostDnsService.h 53151 2014-10-27 12:34:31Z vboxsync $ */
/** @file
 * Host DNS listener.
 */
...
private:
    /*
     * Bit flags to determine what was exactly was changed when
     * Windows triggered event notification.
     */
    enum {
    ...
    /*
     * This structure is used in order to link Windows registry key
     * with an event which is generated once the key has been changed
     * (when interface has updated its DNS setting) or one of the
     * sub-keys has been deleted or added (when interface added or
     * deleted).
     */
    struct Item
    {
        ...
        wchar_t wcsInterface[VBOX_KEY_NAME_LEN_MAX]; /* Path to key within Windows registry. */
    };


    static const wchar_t pwcKeyRoot[];

    /*
     * This flag indicates whether constructor performed
     * initialization correctly.  If not set, monitorWorker() will not
     * perform any action and will be terminated as soon as there will
     * be an attempt to run it.
     */
    bool m_fInitialized;

    /*
     * Keys and events storage.  Size of this vector should not be
     * greater than MAXIMUM_WAIT_OBJECTS because this is exactly
     * maximum amount of events which we allowed to wait for.
     */
    ...
    bool subscribeTo(const wchar_t *wcsPath, const wchar_t *wcsInterface, DWORD fFilter);
    ...
    void extendVectorWithStrings(std::vector<std::string>& pVectorToExtend, const std::wstring &wcsParameter);
