/* $Id: VFSExplorerImpl.cpp 20044 2009-05-26 15:38:55Z vboxsync $ */
/** @file
 *
...
struct VFSExplorer::Data
{
    struct DirEntry
    {
        DirEntry(Utf8Str aName, FileType_T aType)
          : name(aName)
          , type(aType) {}
...
FileType_T VFSExplorer::RTToFileType(int aType) const
{
    FileType_T t;
    switch(aType)
    {
        case RTDIRENTRYTYPE_UNKNOWN: t = FileType_Unknown; break;
        case RTDIRENTRYTYPE_FIFO: t = FileType_Fifo; break;
        case RTDIRENTRYTYPE_DEV_CHAR: t = FileType_DevChar; break;
        case RTDIRENTRYTYPE_DIRECTORY: t = FileType_Directory; break;
        case RTDIRENTRYTYPE_DEV_BLOCK: t = FileType_DevBlock; break;
        case RTDIRENTRYTYPE_FILE: t = FileType_File; break;
        case RTDIRENTRYTYPE_SYMLINK: t = FileType_SymLink; break;
        case RTDIRENTRYTYPE_SOCKET: t = FileType_Socket; break;
        case RTDIRENTRYTYPE_WHITEOUT: t = FileType_WhiteOut; break;
    }
    return t;
}
...
HRESULT VFSExplorer::updateFS(TaskVFSExplorer *aTask)
{
    ...
    try
    {
        ...
        if (RT_FAILURE(vrc))
            throw setError(VBOX_E_FILE_ERROR, tr ("Can't open directory '%s'", pszPath));
        ...
        while (RT_SUCCESS(vrc))
        {
            ...
            if (RT_SUCCESS(vrc))
            {
                ...
                if (name != "." &&
                    name != "..")
                    fileList.push_back(VFSExplorer::Data::DirEntry(name, RTToFileType(entry.enmType)));
            }
        }
        if(aTask->progress)
...
HRESULT VFSExplorer::deleteFS(TaskVFSExplorer *aTask)
{
    ...
    try
    {
             ...
             ++it, ++i)
        {
            ...
            if (RT_FAILURE(vrc))
                throw setError(VBOX_E_FILE_ERROR, tr ("Can't delete file '%s'", szPath));
            if(aTask->progress)
...
HRESULT VFSExplorer::updateS3(TaskVFSExplorer *aTask)
{
    ...
    try
    {
        ...
        if (RT_FAILURE(vrc))
            throw setError(E_FAIL, tr ("Can't open S3 storage service"));
        ...
        if (m->strBucket.isEmpty())
        {
            ...
            if (RT_FAILURE(vrc))
                throw setError(E_FAIL, tr ("Can't get buckets"));

            PCRTS3BUCKETENTRY pTmpBuckets = pBuckets;
            while (pBuckets)
            {
                fileList.push_back(VFSExplorer::Data::DirEntry(pBuckets->pszName, FileType_Directory));
        ...
        else
        {
            ...
            if (RT_FAILURE(vrc))
                throw setError(E_FAIL, tr ("Can't get keys for bucket"));

            PCRTS3KEYENTRY pTmpKeys = pKeys;
            while (pKeys)
            {
                Utf8Str name(pKeys->pszName);
                fileList.push_back(VFSExplorer::Data::DirEntry(pKeys->pszName, FileType_File));
                pKeys = pKeys->pNext;
            }
...
HRESULT VFSExplorer::deleteS3(TaskVFSExplorer *aTask)
{
    ...
    try
    {
             ...
             ++it, ++i)
        {
            ...
            if (RT_FAILURE(vrc))
                throw setError(VBOX_E_FILE_ERROR, tr ("Can't delete file '%s'", (*it).c_str()));
            if(aTask->progress)
                aTask->progress->setCurrentOperationProgress(fPercentStep * i);
        }
    }
...
STDMETHODIMP VFSExplorer::EntryList(ComSafeArrayOut(BSTR, aNames), ComSafeArrayOut(FileType_T, aTypes))
{
    ...
    com::SafeArray<ULONG> sfaTypes((FileType_T)m->entryList.size());
