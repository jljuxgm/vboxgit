/* $Id: tstAnimate.cpp 13755 2008-11-03 15:49:06Z vboxsync $ */
/** @file
...
int main(int argc, char **argv)
{
    ...
    if (VBOX_SUCCESS(rc))
    {
        ...
        PVMREQ pReq1 = NULL;
        if (FileRawMem != NIL_RTFILE)
            rc = VMR3ReqCall(pVM, VMREQDEST_ALL, &pReq1, RT_INDEFINITE_WAIT, (PFNRT)loadMem, 3, pVM, FileRawMem, &offRawMem);
        else
            rc = VMR3ReqCall(pVM, VMREQDEST_ALL, &pReq1, RT_INDEFINITE_WAIT, (PFNRT)SSMR3Load, 4, pVM, pszSavedState, SSMAFTER_DEBUG_IT, NULL, NULL);
        AssertReleaseRC(rc);
        rc = pReq1->iStatus;
        VMR3ReqFree(pReq1);
        if (VBOX_SUCCESS(rc))
        {
            ...
            {
                rc = VMR3ReqCall(pVM, VMREQDEST_ALL, &pReq1, RT_INDEFINITE_WAIT, (PFNRT)scriptRun, 2, pVM, FileScript);
                AssertReleaseRC(rc);
                rc = pReq1->iStatus;
                VMR3ReqFree(pReq1);
            }
            if (VBOX_SUCCESS(rc))
            {
                if (fPowerOn)
                {
                    ...
                    if (VBOX_SUCCESS(rc))
                    {
                        rc = VMR3ReqCall(pVM, VMREQDEST_ALL, &pReq1, RT_INDEFINITE_WAIT, (PFNRT)EMR3RawSetMode, 2, pVM, EMRAW_NONE);
                        AssertReleaseRC(rc);
                        VMR3ReqFree(pReq1);
