/* $Id: PGMR0SharedPage.cpp 40005 2012-02-06 10:47:20Z vboxsync $ */
/** @file
 ...
 * @param   cRegions    Number of regions (local).
 * @param   paRegions    Region array (local).
 */
VMMR0DECL(int) PGMR0SharedModuleCheck(PVM pVM, PGVM pGVM, VMCPUID idCpu, PGMMSHAREDMODULE pModule,
                                      uint32_t cRegions, PGMMSHAREDREGIONDESC paRegions)
{
    int                rc = VINF_SUCCESS;
    ...
    {
        Assert((paRegions[idxRegion].cbRegion & 0xfff) == 0);
        Assert((paRegions[idxRegion].GCRegionAddr & 0xfff) == 0);

        RTGCPTR  GCRegion = paRegions[idxRegion].GCRegionAddr;
        unsigned cbRegion = paRegions[idxRegion].cbRegion & ~0xfff;
        unsigned idxPage  = 0;
                    ...
                    &&  PGM_PAGE_GET_STATE(pPage) == PGM_PAGE_STATE_ALLOCATED)
                {
                    ...
                    if (rc == VINF_SUCCESS)
                    {
                        ...
                        if (PageDesc.idPage != NIL_GMM_PAGEID)
                        {
                            ...
                            Log(("PGMR0SharedModuleCheck: shared page gc virt=%RGv phys %RGp host %RHp->%RHp\n", paRegions[idxRegion].GCRegionAddr + idxPage * PAGE_SIZE, PageDesc.GCPhys, PGM_PAGE_GET_HCPHYS(pPage), PageDesc.HCPhys));
