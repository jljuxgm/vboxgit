/* $Id: TMAll.cpp 20750 2009-06-21 21:14:52Z vboxsync $ */
/** @file
...
VMMDECL(int) TMTimerSetRelative(PTMTIMER pTimer, uint64_t cTicksToNext, uint64_t *pu64Now)
{
     ...
     */
    bool fOwnTMLock = RT_SUCCESS(tmTryLock(pVM));
#if 1
    if (    fOwnTMLock
        &&  pTimer->pCritSect)
    {
...
/** @todo call host context and yield after a couple of iterations */
        ...
        if (!fOwnTMLock)
        {
            fOwnTMLock = RT_SUCCESS(tmTryLock(pVM));
            if (    !fOwnTMLock
                &&  enmClock == TMCLOCK_VIRTUAL_SYNC
                &&  !fOwnVirtSyncLock)
                fOwnVirtSyncLock = RT_SUCCESS(tmVirtualSyncTryLock(pVM));
        }
