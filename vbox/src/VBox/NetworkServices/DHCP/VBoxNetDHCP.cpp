/* $Id: VBoxNetDHCP.cpp 46962 2013-07-04 05:44:31Z vboxsync $ */
/** @file
...
int VBoxNetDhcp::init()
{
        ...
        && fNeedDhcpServer)
    {
        ...
        com::Bstr strGateway;
        hrc = m_NATNetwork->GetGateway(strGateway.asOutParam());
        AssertComRCReturn(hrc, VERR_INTERNAL_ERROR);
    ...
    if (m_fIgnoreCmdLineParameters)
    {
        ...
        hrc = m_DhcpServer->GetGlobalOptions(ComSafeArrayAsOutParam(sf));
        AssertComRCReturn(hrc, VERR_INTERNAL_ERROR);
        ...
        hrc = m_DhcpServer->GetUpperIP(strUpperIp.asOutParam());
        AssertComRCReturn(hrc, VERR_INTERNAL_ERROR);
        RTNetStrToIPv4Addr(com::Utf8Str(strUpperIp).c_str(), &UpperAddress);
        

        hrc = m_DhcpServer->GetLowerIP(strLowerIp.asOutParam());
        AssertComRCReturn(hrc, VERR_INTERNAL_ERROR);
